...
upp:	.word	101	# the upper bound for the rng (exclusive)
...
main:
	...
	la $t0, arr	# init the base address of the array into $t0
	lw $t1, len	# init the size of the array into $t1
	loop:
		beq $t1, $zero, exit	# exit if we finished
		li $v0, 42		# init rng service code
		lw $a1, upp		# init the upper bound of the rng
		syscall
		sw $a0, ($t0)		# save the generated number into the array
		addi $t0, $t0, 4	# update the off-set of the base address
		subi $t1, $t1, 1	# update the size of how many more times we need to add a number to the array
		j loop
	exit:
